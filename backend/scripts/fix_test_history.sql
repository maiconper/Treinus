-- Script de correção: limpa sessões de teste e re-gera histórico PPL
-- Execute com: psql -U treinus -d treinus -h localhost -f fix_test_history.sql
...
BEGIN
  ...
  -- Remove sessões de teste (cascata remove session_exercises e session_sets)
  DELETE FROM training_sessions WHERE user_id = v_user;
  RAISE NOTICE 'Sessões antigas removidas.';
END $$;

-- ── Re-gera histórico PPL (2 meses) ────────────────────────────────────────
...
BEGIN
  ...
  FOR day_off IN 0..59 LOOP
    ...
    dow      := EXTRACT(DOW FROM cur_date)::INTEGER;
    wk       := day_off / 7 + 1;
  ...
  RAISE NOTICE 'Histórico gerado com sucesso!';
END $$;
